...
/**
ECMA119/8.5で規定されるSupplementaryVolumeDescriptorを表現するクラス。

<dl>
<dt>詳細</dt><dd>UDFVライブラリでは現在SupplementaryVolumeDescriptorに対応していないため、ボリューム中に存在しても正しく処理される保証はない。</dd>
<dt>フィールド一覧</dt>
<dd>
<table border="1">
...
<tr><td><b>PublisherId</b></td><td><b>UDF_bytes</b></td><td><i>128</i></td></tr>
...
*/
public class UDF_ECMA119_CD001_2 extends UDF_Element 
{
    ...
    */
	...
	@param name   エレメント名。
    */
    public UDF_ECMA119_CD001_2(UDF_Element elem, String prefix, String name){
    ...
    */
    public int getSize(){
	return 0+1+standardId.getSize()+1+volFlags.getSize()+systemId.getSize()+volId.getSize()+unusedField.getSize()+volSpaceSize.getSize()+escapeSeqs.getSize()+volSetSize.getSize()+volSqeuenceNumber.getSize()+logicalBlockSize.getSize()+pathTableSize.getSize()+4+4+locOfOccurenceOfTypeMPathTable.getSize()+locOfOptionalOccurenceOfTypeMPathTable.getSize()+directoryRecordForRootDirectory.getSize()+volSetId.getSize()+publisherId.getSize()+dataPreparerId.getSize()+applicationId.getSize()+copyrightFileId.getSize()+abstractFileId.getSize()+bibliographicFileId.getSize()+volCreationDateAndTime.getSize()+volModificationDateAndTime.getSize()+volExpirationDateAndTime.getSize()+volEffectiveDateAndTime.getSize()+1+reserved.getSize()+applicationUse.getSize()+reserved2.getSize();
    }
    ...
    */
	...
	@return 取得したvolFlags を返す。
    */
    public UDF_bytes getVolFlags(){return volFlags;}
    /**
	systemIdを取得する。
    ...
    */
	...
	@return 取得したescapeSeqs を返す。
    */
    public UDF_bytes getEscapeSeqs(){return escapeSeqs;}
    /**
	volSetSizeを取得する。
    ...
    */
    ...
    /**
	volSqeuenceNumberを取得する。

	@return 取得したvolSqeuenceNumber を返す。
    */
    public UDF_uint16_lebe getVolSqeuenceNumber(){return volSqeuenceNumber;}
    /**
    ...
    */
	...
	@return 取得したpublisherId を返す。
    */
    public UDF_bytes getPublisherId(){return publisherId;}
    /**
	dataPreparerIdを取得する。
    ...
    */
	...
	@param	v 設定する値。
    */
    public void setVolFlags(UDF_bytes v){replaceChild(v, volFlags); volFlags = v;}
    /**
	systemIdを設定する。
    ...
    */
	...
	@param	v 設定する値。
    */
    public void setEscapeSeqs(UDF_bytes v){replaceChild(v, escapeSeqs); escapeSeqs = v;}
    /**
	volSetSizeを設定する。
    ...
    */
    ...
    /**
	volSqeuenceNumberを設定する。

	@param	v 設定する値。
    */
    public void setVolSqeuenceNumber(UDF_uint16_lebe v){replaceChild(v, volSqeuenceNumber); volSqeuenceNumber = v;}
    /**
    ...
    */
	...
	@param	v 設定する値。
    */
    public void setPublisherId(UDF_bytes v){replaceChild(v, publisherId); publisherId = v;}
    /**
	dataPreparerIdを設定する。
    ...
    */
	...
	escapeSeqs = (UDF_bytes)createElement("UDF_bytes", "ecma119", "escape-seqs");
	escapeSeqs.setSize(32);
	rsize += escapeSeqs.readFrom(f);
	volSetSize = (UDF_uint16_lebe)createElement("UDF_uint16_lebe", "ecma119", "vol-set-size");
	rsize += volSetSize.readFrom(f);
	volSqeuenceNumber = (UDF_uint16_lebe)createElement("UDF_uint16_lebe", "ecma119", "vol-sqeuence-number");
	rsize += volSqeuenceNumber.readFrom(f);
	...
	publisherId = (UDF_bytes)createElement("UDF_bytes", "ecma119", "publisher-id");
	publisherId.setSize(128);
	rsize += publisherId.readFrom(f);
    ...
    */
	...
	for(int i=0 ; i<nl.getLength() ; ++i){
	    ...
	    else if(name.equals("vol-flags")){
		volFlags = (UDF_bytes)createElement("UDF_bytes", "ecma119", "vol-flags");
		volFlags.setSize(1);
		volFlags.readFromXML(child);
	    }
	    ...
	    else if(name.equals("escape-seqs")){
		escapeSeqs = (UDF_bytes)createElement("UDF_bytes", "ecma119", "escape-seqs");
		escapeSeqs.setSize(32);
		escapeSeqs.readFromXML(child);
	    }
	    else if(name.equals("vol-set-size")){
		volSetSize = (UDF_uint16_lebe)createElement("UDF_uint16_lebe", "ecma119", "vol-set-size");
		volSetSize.readFromXML(child);
	    }
	    else if(name.equals("vol-sqeuence-number")){
		volSqeuenceNumber = (UDF_uint16_lebe)createElement("UDF_uint16_lebe", "ecma119", "vol-sqeuence-number");
		volSqeuenceNumber.readFromXML(child);
	    }
	    ...
	    else if(name.equals("publisher-id")){
		publisherId = (UDF_bytes)createElement("UDF_bytes", "ecma119", "publisher-id");
		publisherId.setSize(128);
		publisherId.readFromXML(child);
	    }
    ...
    */
	...
	escapeSeqs = (UDF_bytes)createElement("UDF_bytes", "ecma119", "escape-seqs");
	escapeSeqs.setSize(32);
	escapeSeqs.setDefaultValue();
	volSetSize = (UDF_uint16_lebe)createElement("UDF_uint16_lebe", "ecma119", "vol-set-size");
	volSetSize.setDefaultValue();
	volSqeuenceNumber = (UDF_uint16_lebe)createElement("UDF_uint16_lebe", "ecma119", "vol-sqeuence-number");
	volSqeuenceNumber.setDefaultValue();
	logicalBlockSize = (UDF_uint16_lebe)createElement("UDF_uint16_lebe", "ecma119", "logical-block-size");
    ...
    */
	...
	UDF_ECMA119_CD001_2 dup = new UDF_ECMA119_CD001_2(this, null, getName());
	dup.setDefaultValue();
	...
	dup.setVolFlags((UDF_bytes)volFlags.duplicateElement());
	...
	dup.setEscapeSeqs((UDF_bytes)escapeSeqs.duplicateElement());
	dup.setVolSetSize((UDF_uint16_lebe)volSetSize.duplicateElement());
	dup.setVolSqeuenceNumber((UDF_uint16_lebe)volSqeuenceNumber.duplicateElement());
	...
	dup.setPublisherId((UDF_bytes)publisherId.duplicateElement());
    ...
    */
	...
	a += publisherId.getInfo(indent + 1);
