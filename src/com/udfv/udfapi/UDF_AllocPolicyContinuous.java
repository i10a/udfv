...
/**
   指定の Partition のSpace Bitmapの指定のlbn 以降の
   指定のサイズの連続している空き領域を探して割り当て、開放するクラス。

*/
public class UDF_AllocPolicyContinuous extends UDF_AllocPolicyImpl {

    public UDF_AllocPolicyContinuous(){
        this(0, 0, 1);
    }
    public UDF_AllocPolicyContinuous(int lbn, int partno){
        this(lbn, partno, 1);
    }
    /**
      コンストラクタ。
    ...
    */
    public UDF_AllocPolicyContinuous(int lbn, int partno, int align){
        super(lbn, partno, align);
    }

    /**
       指定の Partition のSpace Bitmapの指定のlbn 以降の指定のサイズの連続の空き領域をだけ探して割り当てる。
...
*/
        ...
        //　空き領域を検索　//
        try {
            lbn = bm.searchBitOn(lbn, default_align, sec);
        }
